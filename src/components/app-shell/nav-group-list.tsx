...
/**
 * Shared accordion nav list used by the desktop sidebar, the mobile drawer, and the
 * mobile bottom-nav's "More" sheet — one place so collapse behavior can't drift between them.
 */
          ...
          return (
            <div key={group.links[0]?.href} className="space-y-0.5">
              {group.links.map((link) => (
                <NavLink key={link.href} href={link.href} label={link.label} size={size} onClick={onLinkClick} />
              ))}
            </div>
          );
        }
