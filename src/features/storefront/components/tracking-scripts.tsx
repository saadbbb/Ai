...
/**
 * No-code tracking setup (PART 13 gap #142) — the workspace owner pastes in
 * IDs from their own Meta/Google/TikTok accounts; this just injects the
 * standard vendor snippet for whichever ones are set. No account of ours
 * involved, nothing to verify server-side.
 */
export function TrackingScripts({ ids }: { ids: Record<string, string> }) {
  return (
    <>
      {ids.metaPixelId && (
...
})(window,document,'script','dataLayer','${ids.googleTagManagerId}');`}
        </Script>
      )}
      {ids.tiktokPixelId && (
