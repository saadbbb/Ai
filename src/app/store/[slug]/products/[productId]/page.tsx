...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!product) return {};
  return { title: `${product.name} — ${row.workspaceName}`, description: product.description ?? undefined };
}
      ...
      <div className="mx-auto max-w-4xl space-y-8 px-6 py-12">
        ...
        <div className="grid gap-8 sm:grid-cols-2">
          <div className="space-y-2">
            {gallery.length > 0 ? (
              <>
                <img src={gallery[0]} alt={product.name} className="aspect-square w-full rounded-lg object-cover" />
                {gallery.length > 1 && (
              ...
              {related.map((item) => (
                <Link key={item.id} href={`/store/${slug}/products/${item.id}`}>
                  <Card className="h-full transition-shadow hover:shadow-md">
                    {item.imageUrl && <img src={item.imageUrl} alt={item.name} className="h-32 w-full rounded-t-lg object-cover" />}
